...
移动到某个历史文件上。这些东西全部由.git的目录在控制，所以，没事千万别乱动那个
.git目录。
	当你把所有修改后的文件commit后，工作区这时也就是干净的了。
##git的本质
...
---------E
	...
	You like git too
-----------S
yubo@debian:~/git$ echo "You like git too" >> readme.txt 
yubo@debian:~/git$ git add readme.txt 
yubo@debian:~/git$ git status
# 位于分支 master
# 要提交的变更：
...
#
----------E
	我们用commit命令提交后，用 git diff HEAD --readme.txt 查看
工作区和版本库里面最新版本的区别。
----------S
yubo@debian:~/git$ git diff HEAD -- readme.txt 
diff --git a/readme.txt b/readme.txt
index fbc2bd2..2c2e039 100644
--- a/readme.txt
+++ b/readme.txt
