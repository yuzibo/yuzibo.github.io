...
## B2U
实际上，B2U<sub>(w)</sub>(x)的含义是 Binary to Unsigned,w代表的是位数。

`UMax`<sub>w</sub> = 2<sup>w</sup> - 1  ，即范围是在[0...0]到[1..1]范围内。
比如，  `UMax`<sub>4</sub> = 2^4 - 1 = 15

## B2T
这里的`T`就是 Two's complement的意思（Binary to T）,其大小要加上最高位的值，"1"表示负的，"0"表示正的。
其公式可以为 B2T<sub>w</sub> = -X<sub>w-1</sub>2<sup>w-1</sup> + sum(from i = 0 to i = w-2) x乘以2^(i)
