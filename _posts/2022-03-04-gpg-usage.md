...
## How
1. 备份： umask 077; tar -cf $HOME/gnupg-backup.tar $HOME .gnupg
2. Create new subkey with signing: 
```bash
vimer@debian-local:~$ gpg --list-keys tsu.yubo@gmail.com
pub   rsa4096 2022-04-09 [SC]
      E2521CB8175736A97052B2F8954E6A70100598A2
...
sub   rsa4096 2022-04-09 [E]
```
3. `gpg --edit-key YOURPRIMARYKEYID` add subkey
```bash
>addkey
gpg> addkey
请选择您要使用的密钥类型：
...
4. 保存目前的 `.gnupg`的目录：  tar -zcvf gnugpg-primary-2022-04-13.tar.gz ~/.gnupg
一定要把目前的tar的保存在离线的U盘或者操作时在离线时进行，真的需要特别谨慎。
5.  删除primary key的keygrip：
```bash
