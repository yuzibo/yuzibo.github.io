...
这样就会打印hash的key，但是有一点请注意，perl不保证输出的顺序，也就是你最好不要期望得到你想要的顺序。又但是，keys 和 values函数可以保证此时的值相对应，例如，上面的例子@k会打印出c b a,那么打印@v一定是3 1 2
...
不知道我的思路是否有问题，$hash,@hash,%hash三者之间有着某种联系虽说hash是用来定义的，但它仅仅是定义吗？在其他的外部调用中，我们反而使用的是$hash{""},在当时学习@hash和$hash也有同样的问题。
